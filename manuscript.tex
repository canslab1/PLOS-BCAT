...
For the MD-A experiment we isolated a coordination failure channel by initializing all non-pioneer agents with a uniform attitude of~81 (avg-of-attitudes~=~80, std-of-attitudes~=~0) and setting the bounded-confidence value to~10. The attitude distance between pioneers (\textit{att}~=~100) and non-pioneers (\textit{att}~=~81) was set to~19, which exceeded the communication threshold so that no opinion exchanges occurred. Since all non-pioneers shared identical attitudes, peer communication among them also failed to produce changes. According to this configuration, an FRI value of~1.0 was guaranteed throughout all simulations, and any adoption failure could be attributed to threshold-based coordination failure. We swept avg-of-thresholds from~10 to~70 in increments of~10, with std-of-thresholds~=~10 and no-of-pioneers~=~5 (clustered).
